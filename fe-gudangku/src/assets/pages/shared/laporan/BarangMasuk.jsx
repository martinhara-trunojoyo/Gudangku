...
import { getStockInReport, exportStockIn } from "../../../_service/laporan";
import { 
    exportToPDF, 
    exportToExcel, 
    formatStockInData, 
...
export default function BarangMasuk() {
    ...
    const loadReportData = async () => {
        try {
            ...
            const response = await getStockInReport(params);
            console.log("Stock In Report Response:", response);
    ...
    const handleExportPDF = async () => {
        try {
            ...
            const response = await exportStockIn(params);
            
            if (response.status === 'success') {
                const formattedData = formatStockInData(response.data);
                const title = `LAPORAN BARANG MASUK${dateRange.startDate && dateRange.endDate 
                ...
                exportToPDF(
                    ...
                    stockInColumns, 
                    title,
                    `laporan_barang_masuk_${new Date().toISOString().slice(0, 10)}.pdf`
                );
        ...
        try {
            ...
            if (response.status === 'success') {
                ...
                exportToExcel(
                    ...
                    stockInColumns, 
                    title,
                    response.filename || `laporan_barang_masuk_${new Date().toISOString().slice(0, 10)}.xlsx`
                );
            ...
            <div className="container mx-auto px-6 py-8 relative z-10">
                ...
                {summary && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow p-4">
                            <h3 className="text-sm font-medium text-gray-500">Total Barang Masuk</h3>
                            <p className="text-2xl font-bold text-green-600">{summary.total_quantity || 0}</p>
                        </div>
                        <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow p-4">
                                ...
                                {reportData.length > 0 ? (
                                    reportData.map((item, index) => (
                                        <tr key={item.id || index} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="py-3 px-4">{item.jumlah_masuk} {item.barang?.satuan || ''}</td>
                                            <td className="py-3 px-4">{formatDate(item.tanggal_masuk)}</td>
                                            <td className="py-3 px-4">{item.supplier?.nama_supplier || '-'}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
